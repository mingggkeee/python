# 연락처 관리 프로그램
# 생성(C), 조회(R), 수정(U), 삭제(D) -> CRUD
# 연락처 데이터 내용 : 번호(자동증가처리), 이름, 전화번호, 이메일
# 연락처 데이터 형식 : 리스트 or 딕셔너리 -> 딕셔너리 사용 -> {"no": 1, "name": 'id', "email": 'e', "phone": 'p' }
...
class ContactManager:
    ...
    def search_contacts(self):
        ...
        # 연락처 목록을 순회하면서 입력받은 이름과 일치하는 이름의 연락처 찾기  abc == abc / abc == a~, ~b~ 부분 일치 검색
        for c in self.contacts:
